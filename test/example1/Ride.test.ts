...
test("Deve calcular o valor da corrida em horário normal", function () {
    ride.addSegment(10,new Date("2022-03-01T10:00:00"));
    const fare = ride.finish();
    expect(fare).toBe(21);
});

test("Deve calcular o valor da corrida em horário noturno", function () {
    ride.addSegment(10, new Date("2022-03-01T23:00:00"));
    const fare = ride.finish();
    expect(fare).toBe(39);
});

test("Deve calcular o valor da corrida no domingo", function () {
    ride.addSegment(10, new Date("2022-03-06T10:00:00"));
    const fare = ride.finish();
    expect(fare).toBe(29);
});

test("Deve calcular o valor da corrida no domingo a noite", function () {
    ride.addSegment(10, new Date("2022-03-06T23:00:00"));
    const fare = ride.finish();
    expect(fare).toBe(50);
});

test("Deve calcular o valor da corrida mínima", function () {
    ride.addSegment(3, new Date("2022-03-01T10:00:00"));
    const fare = ride.finish();
    expect(fare).toBe(10);
});

test("Deve retornar -1 se a distância for invállida", function () {
    expect(() => ride.addSegment(-3, new Date("2022-03-01T10:00:00"))).toThrow(new Error("Invalid Distance"));
});

test("Deve retornar -2 se a data for invállida", function () {
    expect(() => ride.addSegment(3, new Date("abcdef"))).toThrow(new Error("Invalid Date"));
});

test("Deve calcular o valor da corrida em múltiplos horários", function () {
    ride.addSegment(10, new Date("2022-03-01T21:00:00"));
    ride.addSegment(10, new Date("2022-03-01T22:00:00"));
    const fare = ride.finish();
    expect(fare).toBe(60);
});